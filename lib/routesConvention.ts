 ...
 */
export function defineConventionalRoutes(appDir: string, ignoredFilePatterns?: string[]): RouteManifest {
  ...
  visitFiles(path.join(appDir, "routes"), (file) => {
    ...
    throw new Error(`Invalid route module file: ${path.join(appDir, "routes", file)}`);
  });
  ...
  function defineNestedRoutes(defineRoute: DefineRouteFunction, parentId?: string): void {
    ...
    for (let routeId of childRouteIds) {
      let routePath: string | undefined = createRoutePath(routeId.slice((parentId || "routes").length + 1));
