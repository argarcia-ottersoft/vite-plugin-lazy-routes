...
import type { ConfigRoute, DefineRouteFunction, DefineRouteOptions, RouteManifest } from "./routes";
...
export function flatRoutes(appDirectory: string, ignoredFilePatterns?: string[]): RouteManifest {
...
export function flatRoutesUniversal(
  ...
  function defineNestedRoutes(defineRoute: DefineRouteFunction, parentId?: string): void {
    ...
    for (let childRoute of childRoutes) {
      ...
      if (childRoute.index) {
        let invalidChildRoutes = routes.filter((routeInfo) => routeInfo.parentId === childRoute.id);
...
function findParentRouteId(routeInfo: RouteInfo, nameMap: Map<string, RouteInfo>) {
...
export function getRouteInfo(appDirectory: string, routeDirectory: string, filePath: string): RouteInfo {
...
export function createRoutePath(routeSegments: string[], rawRouteSegments: string[], isIndex: boolean) {
  let result = "";
...
function getRouteMap(appDirectory: string, routePaths: string[], prefix: string): Readonly<Map<string, RouteInfo>> {
  ...
  for (let routePath of routePaths) {
    ...
    if (isRouteModuleFile(pathWithoutAppRoutes)) {
      ...
      let uniqueRouteId = (routeInfo.path || "") + (routeInfo.index ? "?index" : "");
...
function isRouteInFolder(routeId: string) {
  return (routeId.endsWith("/index") || routeId.endsWith("/route")) && routeId.includes(path.posix.sep);
}
...
export function getRouteConflictErrorMessage(pathname: string, routes: string[]) {
